// Package forailapi is a thin HTTP client over the Forail REST API.
//
// Auth: HTTP Basic (admin user) for now. A token-based path can be added
// later — Forail supports OAuth2 PATs via /api/v2/tokens/.
//
...
type Client struct {
	BaseURL string
	// Token is an OAuth2 Bearer (PAT) issued via `forail-manage
	// create_oauth2_token` or POST /api/v2/users/{id}/personal_tokens/.
	// Forail does not accept HTTP Basic on /api/v2/.
	Token string
	// HostHeader overrides the Host header on outbound requests. Needed
	// when reaching Forail through an Ingress that routes by hostname
	// (e.g. forail.local) but the BaseURL uses a node IP.
	HostHeader string
	HTTP       *http.Client
}
...
func (e *errAPI) Error() string {
	return fmt.Sprintf("forail api %s: %d: %s", e.URL, e.Status, e.Body)
}
...
// listResult is the standard Forail list envelope.
