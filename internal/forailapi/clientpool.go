// ClientPool resolves a *forailapi.Client per ForailInstance CR.
//
// Multi-cluster sync works by giving each managed CR an optional
// `spec.forailInstance` field. When set, the controller calls
// `pool.For(ctx, namespace, name)` which:
//   1. fetches the ForailInstance CR from the manager's k8s cache,
...
//
// When forailInstance is empty the controller uses the pool's Default
// client, which is the global Forail backend supplied via flags / env.
package forailapi
...
// ClientPool dispenses per-ForailInstance clients backed by a k8s reader.
//
// Default is the global / fallback client used when CRs do not reference
// a specific ForailInstance. K8s is the controller-runtime client used to
// fetch ForailInstance CRs and referenced Secrets.
...
// For returns a Client for the given ForailInstance CR (namespace + name).
// Empty name returns the default client.
func (p *ClientPool) For(ctx context.Context, namespace, name string) (*Client, error) {
	if name == "" {
		if p.Default == nil {
			return nil, errors.New("no default Forail client configured")
		}
		return p.Default, nil
	}
	if p.K8s == nil {
		return nil, fmt.Errorf("ClientPool has no k8s reader; cannot resolve ForailInstance %q", name)
	}

	var fi forailv1.ForailInstance
	if err := p.K8s.Get(ctx, types.NamespacedName{Namespace: namespace, Name: name}, &fi); err != nil {
		return nil, fmt.Errorf("get ForailInstance %s/%s: %w", namespace, name, err)
	}
...
// ForailInstance controller after spec changes so the next For() rebuilds.
...
// ForailInstance reconciler to update status.
