...
// mockForail is a tiny stateful HTTP server that mimics the subset of the
// Forail REST API the operator uses. Each resource type is a map keyed by
// numeric ID. Tests assert against this state to verify reconcile.
type mockForail struct {
	mu sync.Mutex
...
func (m *mockForail) CallCount(handlerKey string) int {
	m.mu.Lock()
	defer m.mu.Unlock()
	return m.calls[handlerKey]
}

func (m *mockForail) start(t *testing.T) (*httptest.Server, *forailapi.Client) {
	t.Helper()
	srv := httptest.NewServer(http.HandlerFunc(m.handle))
	t.Cleanup(srv.Close)
	return srv, newTestForailClient(srv.URL, "test-token")
}

func newTestForailClient(baseURL, token string) *forailapi.Client {
	return forailapi.New(baseURL, token, "", true)
}

// newTestClientPool builds a ClientPool wrapping the default client and
// the manager's k8s client (used to look up ForailInstance CRs in tests
// that exercise the multi-cluster path).
func newTestClientPool(def *forailapi.Client, k client.Client) *forailapi.ClientPool {
	return forailapi.NewClientPool(def, k)
}

// listEnvelope produces the Forail `{count, results}` shape.
...
func (m *mockForail) handle(w http.ResponseWriter, r *http.Request) {
	m.mu.Lock()
	defer m.mu.Unlock()
	...
	switch {
	// --- ping (health probe used by ForailInstance) ---
...
func (m *mockForail) handleInventoryNested(w http.ResponseWriter, r *http.Request, path string) {
...
func (m *mockForail) handleGroupNested(w http.ResponseWriter, r *http.Request, path string) {
...
func (m *mockForail) handleJobTemplateNested(w http.ResponseWriter, r *http.Request, path string) {
