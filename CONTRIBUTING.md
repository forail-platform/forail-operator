# Contributing to forail-operator
...
Kubernetes operator that reconciles Forail resources (`Organization`, `Team`, `Project`, `Workflow`, `Inventory`, `Credential`, `JobTemplate`, `Schedule`, `ForailInstance`) against one or more Forail API endpoints.
...
```bash
git clone https://github.com/forail-platform/forail-operator.git
cd forail-operator
...
- **Multi-cluster routing** — when modifying `forailapi.ClientPool`, preserve cache invalidation on `spec.forailInstance` change.
- **Version bumps** — operator version, Chart.yaml, helm values `tag`, and CSV version must all agree.

## Reporting bugs

Open an issue with reproduction steps, operator + Kubernetes versions, and reconcile logs (`kubectl logs -n forail-operator`).
